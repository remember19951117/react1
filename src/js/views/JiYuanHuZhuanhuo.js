...
class JiYuanHuZhuan extends Component {
	...
	onClickTabs() {
		...
		const context = this;
		// this.type = 1;
		zhuanzhang(1, 1, this.state.page, this.state.pageSize).then(function (result) {
			// //console.log(result)
			if (result.status) {
	...
	onClickTabs2() {
		...
		const context = this;
		zhuanzhang2(1, 2, this.state.page, this.state.pageSize).then(function (result) {
			console.log(result)
			if (result.status) {
				context.setState({ list: result.resource, sum: result.sum, zongyeshu: parseInt(result.sum / 20) + 1 });
			}
	...
	onClickBTN() {
		...
		if (this.refs.sum.value <= 0) { $('.alertMessage').text("积分不足不足，转账失败"); $('#my-alert').modal('open'); return; }
	...
	Next() {
		console.log(1);
		// console.log( "this.state.qu",this.state.qu)
	...
	render() {
		...
		return (
			<div style={{ background: '#f3f1f2' }}>
				...
				<div className="DT_top"><Link to="/nav"><span className="DT_left">《</span></Link><span className="DT_right">{lang.nav33}</span></div>
					...
					<div className="am-tabs-bd">
						<div className="am-tab-panel am-active">
							...
							<div className="XG_content">
								...
								<div className="XG_btn">

									<input type="button" value={lang.JiYuanHuZhuan08} onClick={this.onClickBTN.bind(this)} />
								</div>
							</div>
						</div>
