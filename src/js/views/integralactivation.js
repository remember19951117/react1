...
class integralactivation extends Component {
	state = {
		list: [],
		pageSize: 10000,
		page: 1,
	...
	componentWillMount() {
		const { integralactivationList } = this.props;
		const context = this;
		integralactivationList(this.state.page, this.state.pageSize).then(function (result) {
			// //console.log(result);
			if (result.status) {
		...
		return (
			...
			`}
				...
				<div className="GR_top">
					...
					<span className="GR_zc">激活积分明细</span>
				</div>
				{
...
integralactivation.contextTypes = {
...
export default connect(mapStateToProps, mapDispatchToProps)(integralactivation)
